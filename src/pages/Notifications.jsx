...
const typeMeta = {
  ...
  warning: { icon:AlertCircle,   bg:'bg-warning-soft', color:'text-warning-text', tone:'warning' },
  success: { icon:CheckCircle2,  bg:'bg-success-soft', color:'text-success-text', tone:'success' },
...
const NotificationItem = ({ n, onToggleRead, onDismiss }) => {
  ...
  return (
    <div className={`group relative flex gap-3 px-5 py-4 hover:bg-surface-2 transition-colors cursor-pointer ${!n.isRead ? 'bg-surface-2' : ''}`}>
  ...
  return (
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-2">
        ...
        <span className={`text-[12.5px] font-medium ${on ? 'text-ink' : 'text-ink-muted'}`}>{label}</span>
      </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-4">
      <Card className="overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-line-soft">
          <div>
            <h2 className="text-[16px] font-bold text-ink tracking-tight" style={{ fontFamily:'Plus Jakarta Sans' }}>
              Activity feed
            </h2>
            <p className="text-[11.5px] text-ink-muted mt-0.5">
              {counts.unread > 0
                ? <><span className="font-semibold text-primary-text">{counts.unread} unread</span> · {counts.all} total</>
          ...
          {[
            ...
            <button key={t.id} onClick={() => setFilter(t.id)}
              className={`shrink-0 inline-flex items-center gap-1.5 h-8 px-3 rounded-lg text-[12px] font-semibold transition-colors ${
                filter===t.id ? 'bg-tooltip text-white' : 'text-ink-soft hover:bg-surface hover:text-ink'
              }`}>
              {t.label}
        ...
        <div>
          {filtered.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <p className="text-[12.5px] text-ink-muted mt-1">No notifications matching this filter.</p>
            </div>
          ) : (
              ...
              return (
                <div key={group}>
                  <div className="px-5 py-2 bg-surface-2 border-y border-line-soft flex items-center justify-between">
                    <span className="text-[10.5px] font-bold uppercase tracking-wider text-ink-muted">{group}</span>
      ...
      <div className="space-y-3">
        <Card className="p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <p className="text-[13px] font-bold text-ink" style={{ fontFamily:'Plus Jakarta Sans' }}>Today's digest</p>
          </div>
          <ul className="space-y-2.5 text-[12.5px]">
            <li className="flex items-start gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-danger mt-1.5 shrink-0"/>
              <span className="text-ink"><span className="font-semibold">1 critical</span> inventory alert needs action</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-success mt-1.5 shrink-0"/>
              <span className="text-ink">Daily target hit <span className="font-semibold tabular-nums">2h earlier</span> than usual</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-primary mt-1.5 shrink-0"/>
              <span className="text-ink"><span className="font-semibold">2 AI insights</span> waiting for review</span>
            </li>
          </ul>
        </Card>

        <Card className="p-4">
          <p className="text-[13px] font-bold text-ink mb-3" style={{ fontFamily:'Plus Jakarta Sans' }}>Delivery channels</p>
        ...
        <Card className="p-4">
          <p className="text-[13px] font-bold text-ink mb-1.5" style={{ fontFamily:'Plus Jakarta Sans' }}>Quiet hours</p>
